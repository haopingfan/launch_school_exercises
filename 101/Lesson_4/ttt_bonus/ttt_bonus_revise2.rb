...
def two_marker?(marker, line, brd)
  marker_in_this_row = brd.values_at(*line)
  marker_in_this_row.count(marker) == 2
end

def empty_square_in(line, brd)
  empty_squares = brd.select do |index, value|
    line.include?(index) && value == INITIAL_MARKER
  end
  empty_squares.keys.first # [].first => nil [1].first => 1
end

def find_win_square(marker, brd)
  WINNING_LINES.each do |line|
    if two_marker?(marker, line, brd)
      empty_square = empty_square_in(line, brd)
